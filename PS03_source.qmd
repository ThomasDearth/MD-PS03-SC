...
-   The last city displayed in txhousing is **`r tail(txhousing, 1)$city`**.
:::
...
-   This represents the number of listings in the city at that time.
:::
...
glimpse(txhousing)
ifelse(sum("inventory" %in% c(names(txhousing))) == 0, "Variable removed", "Variable not removed")
```
:::
...
"Due to the order of operator precedence, the example provided provides either (housing which is in Dallas an in 2012) or (any housing which was in 2013). AND operators occur before OR ones."
```
:::
...
```{r}
dallas_sub |> mutate(prct_sold = (sales/listings * 100)) -> dallas_sub
head(dallas_sub, 6)
```
:::
...
```{r}
# determine the mean percentage of sales in each month
dallas_sub |>
  group_by(month) |>
  summarize(mean_prct_sold = mean(prct_sold)) -> dallas_summary

# display dallas_summary
dallas_summary
```
:::
...
-   This code chunk does the same computations as problems four through seven in a single line of executable R code. It pipes txhousing though consecutive dplyr operations to do this. First, it filters the data frame for only rows which are in years 2012 or 2013 and are in Dallas, then uses mutate to create a prct_sold column reflecting the percent of listings in each record which were sold. This is then piped into group_by to separate results by month, followed by arranging them in descending order and using `kable()` to display them in a nice table.
:::
...
txhousing |>
  ...
  group_by(city) |>
  arrange(listings) -> txhousing_by_listing

txhousing_by_listing
```

-   The city of `{r} head(txhousing_by_listing, 1)$city` had the fewest listings, with only `{r} head(txhousing_by_listing, 1)$listings` listings.
:::
...
txhousing |>
  ...
  arrange(desc(total_sales)) -> sales_2012

sales_2012
```

-   In 2012, the most Texan houses were sold during month `{r} head(sales_2012, 1)$month`. In that month, `{r} head(sales_2012, 1)$total_sales` houses were sold.
:::
